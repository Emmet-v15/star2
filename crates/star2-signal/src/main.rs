        ...
        ClientMsg::Stats {
            ...
            eprintln!(
                "[stats] {name}(s{me}) path={path} tx={tx_pps}/s mic={mic_db:.0}dB \
                 ...
                 buf={buf_ms}ms out={out_ms}ms                  rtt={rtt_ms:.1}ms devin={dev_in_ms:.1}ms ring={in_ring_ms:.2}ms enc={enc_ms:.2}ms                  jb={jb_ms:.1}ms devout={dev_out_ms:.1}ms txpath={tx_path_ms:.1}ms rxpath={rx_path_ms:.1}ms                  contract={contract_pps}/s"
            );
        }
